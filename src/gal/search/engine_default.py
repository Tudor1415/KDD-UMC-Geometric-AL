"""Default (distance-focused) branch-and-bound search implementation."""
    ...
    *,
    ...
    while heap and (math.isinf(tau) or best_distance > tau + eps):
        ...
        if a_leaf and b_leaf:
            ...
            if a is b:
                pair, dist, evals, _ = exact_leaf_self(a, context)
            else:
                pair, dist, evals, _ = exact_leaf_eval(a, b, context)
    ...
    if best_pair is None and not heap:
        ...
        while stack and not done:
            ...
            if node_is_leaf(node):
                pair, dist, evals, _ = exact_leaf_self(node, context)
                stats["objective_evals"] = int(stats["objective_evals"]) + evals
