    ...
    *Sparse* Bayesian network learned with pgmpy, but evaluated on the GPU
    ...
    def fit(
        ...
        *,
        ...
        """Learn a sparse Bayesian network from binary transactions.

        Fast-path tricks used in this implementation:

        * **Row subsampling** (``structure_sample``) – limit the number of
          rows considered during the structure search; CPDs are still fitted on
          the full dataset.
        * **Good starting point** (``use_chow_liu_start``) – initialise the
          hill-climbing search with a Chow–Liu tree instead of an empty graph.
        * **Early convergence** (``max_iter`` and ``epsilon``) – stop the
          optimisation when improvements become negligible.

        Parameters are exposed so callers can trade speed against accuracy.
        """
        # 1 ────────────────────────────────────────────────────────────────
